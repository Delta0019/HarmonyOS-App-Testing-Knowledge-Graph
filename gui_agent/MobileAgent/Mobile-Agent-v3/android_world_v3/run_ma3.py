...
def _find_adb_directory() -> str:
    ...
    potential_paths = [
        ...
        r"F:\softwares\Android\Sdk\platform-tools\adb.exe",
    ]
    for path in potential_paths:
        if os.path.isfile(path):
            return path
    raise EnvironmentError(
        "adb not found. Please install Android SDK and ensure adb is in PATH."
    )
...
# UTG specific.
_USE_UTG = flags.DEFINE_boolean(
    'use_utg',
    False,
    'Whether to enable UTG (UI Transition Graph) for action hints and graph building.',
)
_UTG_PATH = flags.DEFINE_string(
    'utg_path',
    '',
    'Path to save/load UTG data. If empty, defaults to output_path/utg.json.',
)
# KG (Knowledge Graph) specific.
_USE_KG = flags.DEFINE_boolean(
    'use_kg',
    False,
    'Enable Knowledge Graph for macro-level path planning.',
)
_KG_DATA_DIR = flags.DEFINE_string(
    'kg_data_dir',
    '',
    'Directory containing pre-built KG data.',
)
_KG_PROJECT_PATH = flags.DEFINE_string(
    'kg_project_path',
    '',
    'Path to HarmonyOS-App-Testing-Knowledge-Graph project for imports.',
)
_EMBEDDING_MODEL = flags.DEFINE_string(
    'embedding_model',
    'paraphrase-multilingual-MiniLM-L12-v2',
    'Sentence-transformer model for KG embeddings.',
)
...
def _get_agent(
    ...
    # Mobile Agent v3.
        ...
        agent = mobile_agent_v3.MobileAgentV3_M3A(
            ...
            use_utg=_USE_UTG.value,
            utg_path=_UTG_PATH.value,
            use_kg=_USE_KG.value,
            kg_data_dir=_KG_DATA_DIR.value,
            kg_project_path=_KG_PROJECT_PATH.value,
